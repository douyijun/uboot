 ...
 */
static void nand_command_random(struct mtd_info *mtd, unsigned int command, int column)
{
...
#elif (define CONFIG_SYS_S3C_NAND_HWECC_8BIT)
static int nand_read_page(struct mtd_info *mtd, int block, int page, uchar *dst)
{
        struct nand_chip *this = mtd->priv;
        u_char ecc_calc[ECCTOTAL];
	...
	int col = 0;
        ...
        nand_command(mtd, block, page, 0, NAND_CMD_READ0);
	nand_command_random(mtd, NAND_CMD_RNDOUT, 4096);
	chip->read_buf(mtd, oob_data, mtd->oobsize);

	for (i = 0; eccsteps; eccsteps--, i += eccbytes, p += eccsize) {
		nand_command_random(mtd, NAND_CMD_RNDOUT, col);	
			...
			return;
       	 	...
       	 	col = eccsize * ((4096 / eccsize) + 1 - eccsteps);
    	}
}
